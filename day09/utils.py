...
############### DAYS 2, 5, 7, 9 - will probably be heavily used ################
...
class IntCodeComputer:
    def __init__(self, codes: List) -> None:
        self.raw_input = codes[:]
        self.code = defaultdict(int, enumerate(codes))
        self.inputs = []
    ...
    def get_value(self, arg: int, mode: int) -> int:
        return self.mode_options[mode](arg)

    def position_mode(self, arg: int) -> int:
        return self.code[arg]

    def immediate_mode(self, arg: int) -> int:
        return arg

    def relative_mode(self, arg: int) -> int:
        return self.code[arg + self.relativebase]
    ...
    def debug_print(self, op_name, message):
        if self.debug:
    ...
    def op_add(self) -> None:
        arg1, arg2, pos = self.code[self.cursorPos], self.code[self.cursorPos + 1], self.code[self.cursorPos + 2]

        self.debug_print('add', f"value({self.code[pos]}) in pos({pos}) -> {self.get_value(arg1, self.modes[0]) + self.get_value(arg2, self.modes[1])}")

        first = self.get_value(arg1, self.modes[0])
        second = self.get_value(arg2, self.modes[1])

        self.code[pos] = first + second
        self._inc(3)

    def op_mutliplication(self) -> None:
        arg1, arg2, pos = self.code[self.cursorPos], self.code[self.cursorPos + 1], self.code[self.cursorPos + 2]

        self.debug_print('multiply', f"value({self.code[pos]}) in pos({pos}) -> {self.get_value(arg1, self.modes[0]) * self.get_value(arg2, self.modes[1])}")

        self.code[pos] = self.get_value(arg1, self.modes[0]) * self.get_value(arg2, self.modes[1])
        self._inc(3)
    ...
    def op_input(self) -> None:
        ...
        try:
            ...
            self.code[pos] = self.default_input
    ...
    def op_output(self) -> int:
        ...
        self.debug_print('output', f"adding({self.get_value(pos, self.modes[0])}) to last item in outputs")

        self.outputs.append(self.get_value(pos, self.modes[0]))
        self._inc()

    def op_jumpiftrue(self) -> None:
        arg1, arg2 = self.code[self.cursorPos], self.code[self.cursorPos + 1]

        self.cursorPos += 2

        if self.get_value(arg1, self.modes[0]) != 0:
            self.debug_print('jumpiftrue', f"jump({self.get_value(arg2, self.modes[1])})")
            self.cursorPos = self.get_value(arg2, self.modes[1])

    def op_jumpiffalse(self) -> None:
        arg1, arg2 = self.code[self.cursorPos], self.code[self.cursorPos + 1]
        self.cursorPos += 2

        self.debug_print('jumpiffalse', f"jump({self.get_value(arg2, self.modes[1])})")

        self.cursorPos = self.get_value(arg2, self.modes[1]) if self.get_value(arg1, self.modes[0]) == 0 else self.cursorPos

    def op_lessthan(self) -> None:
        arg1, arg2, pos = self.code[self.cursorPos], self.code[self.cursorPos + 1], self.code[self.cursorPos + 2]

        self.debug_print('lessthan', f"value({self.code[pos]}) in pos({pos}) -> {1 if self.get_value(arg1, self.modes[0]) < self.get_value(arg2, self.modes[1]) else 0}")

        self.code[pos] = 1 if self.get_value(arg1, self.modes[0]) < self.get_value(arg2, self.modes[1]) else 0
        self._inc(3)

    def op_equals(self) -> None:
        arg1, arg2, pos = self.code[self.cursorPos], self.code[self.cursorPos + 1], self.code[self.cursorPos + 2]

        self.debug_print('equals', f"value({self.code[pos]}) in pos({pos}) -> {1 if self.get_value(arg1, self.modes[0]) == self.get_value(arg2, self.modes[1]) else 0}")
    ...
    def op_relativebase(self):
        arg1 = self.cursorPos
        self.debug_print('relativebase', f"modifying relbase value({self.relativebase}) -> {self.relativebase + self.get_value(arg1, self.modes[0])}")
        self.relativebase += self.get_value(arg1, self.modes[0])
        self._inc()

    def op_kill(self):
        raise KillIntCodeComputer
...
def test_intcodecomputer():
    ...
    # result = IntCodeComputer(sample3).set_debugger().set_input(1).run()
    # assert(result[-1] == 16209841)
